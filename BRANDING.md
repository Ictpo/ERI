...
The desktop app takes a beat to boot, so the window opens directly on the
animated "pounce" splash — `SPLASH_HTML` in `backend/app/splash.py`, generated
by `packaging/make_splash_html.py` (self-contained HTML+CSS, fox mark inlined
as a data URI, Matrix-style binary rain). A background worker imports the
analysis stack and starts the server; when `/api/health` is up it swaps the
window to the app URL. Wiring lives in `backend/desktop.py`.

There is **no** PyInstaller `--splash` static image: it showed during the
onefile unpack (before the window exists) but looked out of place against the
animated window, so it was dropped — the unpack phase now shows nothing.

Regenerate after changing the mark or the animation:
